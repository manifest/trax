...
class AcceleratedOptimizerTest(absltest.TestCase):
  ...
  def test_run_simple_task(self):
    ...
    accelerated_opt = optimizers.AcceleratedOptimizer(
        loss_layer, optimizers.SGD(.01))
    rng = fastmath.random.get_prng(0)
    accelerated_opt.update(labeled_batch, rng)
